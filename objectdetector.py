...
cap = cv2.VideoCapture('demo_video.MOV')
...
while True:
    ...
    if len(classIds) != 0:
        ...
        for i in indices:
            ...
            cv2.putText(frame, classNames[0] + " " + confidence, (x + 10, y + 20),
                        font, 1, color, 2)
    ...
    for (face_x, face_y, face_w, face_h) in Faces:
        if face_width_in_frame != 0:
            ...
            Distance = round(Distance * 0.0254, 2)

            # Drwaing Text on the screen
            Distance_level = int(Distance)

            cv2.putText(frame, f"Distance {Distance} Meter",
                        (face_x - 9, face_y - 6), fonts, 0.7, (BLACK), 2)
            if Distance_level < 2.2:
                frame = cv2.putText(frame, "Warning: Entering Into Danger Zone!", (100, 100), cv2.FONT_HERSHEY_SIMPLEX,
                                    1, [0, 0, 255], 4)
